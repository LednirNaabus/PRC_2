                ...
                <div class="shadow overflow-hidden sm:rounded-md p-2 bg-white">
                        ...
                        <div class="px-4 py-2 bg-white sm:p-2 col-span-2">
                            ...
                            <select name="client_id" id="client_id" class="form-input rounded-md shadow-sm mt-1 block text-gray-900 w-full p-2" required>
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}" @if ($client->id==old('client_id','')) selected @endif>{{ strtoupper($client->client_name) }}</option>
                                @endforeach        
                            </select>    
                            @error('client_id')
                    ...
                    <div date-rangepicker class="grid md:grid-cols-2 sm:grid-cols-12 text-white text-sm leading-6">
                        ...
                        <div class="px-4 py-2 bg-white sm:p-2">
                            ...
                            <input name="start" type="text" class="form-input rounded-md shadow-sm mt-1 block text-gray-900 w-full" placeholder="Select start date" value="{{ old('start') }}" required readonly>
                            @error('start')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-2 bg-white sm:p-2">
                            ...
                            <input name="end" type="text" class="form-input rounded-md shadow-sm mt-1 block text-gray-900 w-full" placeholder="Select date end" value="{{ old('end') }}" required readonly>
                            @error('end')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
